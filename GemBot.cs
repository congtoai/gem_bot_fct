...
namespace bot
{
    public class GemBot : BaseBot
    {
        ...
        public List<HeroIdEnum> carry = new List<HeroIdEnum>() { HeroIdEnum.THUNDER_GOD, HeroIdEnum.MERMAID, HeroIdEnum.SEA_GOD }; //2
        ...
        public List<HeroIdEnum> imba = new List<HeroIdEnum>() { HeroIdEnum.CERBERUS };//0
        internal void Load()
        {
        ...
        protected override void StartTurn(ISFSObject paramz)
        {
            ...
            try
            {
                if (heroesFullMana.Any(x => buff.Contains(x.id)))
                {
                    ...
                    if ((anyEnemyFireHero && heroesFullMana.FirstOrDefault().attack < 15) | myAIRSPIRIT == null)
                    {
                    ...
                    {
                        if (myAIRSPIRIT?.isFullMana() == true)
                        {
                            TaskSchedule(delaySwapGem, _ => SendCastSkill(heroesFullMana.FirstOrDefault(), myAIRSPIRIT));
                            return;
                        }
                        else
                        {
                            if (myAIRSPIRIT.attack <= 6)
                            {
                                TaskSchedule(delaySwapGem, _ => SendCastSkill(heroesFullMana.FirstOrDefault(), myAIRSPIRIT));
                                return;
                            }
                ...
                foreach (var heroFullMana in heroesFullMana)
                {
                    ...
                    if (ap.Contains(heroFullMana.id))
                    {
                        if (heroImba != null)
                        {
                            if (heroImba.attack > 10 | !anyHeroBuff)
                            {
                        ...
                        else
                        {
                            ...
                            if (enemyHeroMaxAttack.attack > 10 | !anyHeroBuff )
                            {
                        ...
                        if (enemyheroesAlive.Count() == 1)
                        {
                            TaskSchedule(delaySwapGem, _ => SendCastSkill(heroFullMana));
                            return;
                        }

                        if (heroFullMana.hp < 10)
                        {
                            TaskSchedule(delaySwapGem, _ => SendCastSkill(heroFullMana, enemyHeroMaxAttack));
                            return;
                        }
